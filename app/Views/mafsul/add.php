        ...
        <div class="card">
            <div class="card-content">
                <div class="card-body">
                    ...
                    <hr>
                    <?= form_open('mafsul/create') ?>
                    <div class="col-12">
                        <label for=""><?= lang('app.addReason') ?>:</label>
                        <input type="text" class="form-control" placeholder="<?= lang('app.addReason') ?>" name="reason"><br>
                        <label for=""><?= lang('app.addInfoMafsul') ?>:</label>
                        <textarea name="info" id="" cols="30" rows="10" class="form-control" placeholder="<?= lang('app.addInfoMafsul') ?>"></textarea>
                        <input type="hidden" name="student_id" value="<?= $user['id'] ?>">
                        <input type="hidden" name="class_id" value="<?= $user['level'] ?>">
                        <input type="button" value="<?= lang('app.send') ?>" id="fasl" class="btn btn-lg btn-primary btn-block btn-lg my-2">
                    </div>
                    </form>
                </div>
            </div>
        ...
        <div class="card">
            <div class="card-content">
                <div class="card-body">
                    <div class="col-12">
                        <div class="media">
                            <img src="<?= $user['image'] != null ? base_url($user['image']) : 'https://ui-avatars.com/api/?name=' . $_SESSION['name_ar'] . '&background=random&length=1&font-size=1' ?>" alt="avatar" id="img" class="users-avatar-shadow rounded-circle" height="250" width="250">
                        </div>
                    </div>
                </div>
            </div>
